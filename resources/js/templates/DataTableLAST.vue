...
import FormSelectHorizontal from '../components/FormSelectHorizontal.vue'
...
export default {
  ...
  components: {
    Icon, FormInputHorizontal, FormSelectHorizontal, BasicButton, FormButton, Notification
  },
  setup(props) {
    const filters = ref({});
    const sortBy = ref('-id');
    ...
    return {
      ...
      filters, filterData, changePageDisplayed, pagination, redirectTo, isLoading, resetFilters
    }
  }
}
</script>
<template>
  <div class="p-2">
    ...
    <div class="flex items-center justify-end space-x-3 py-2">
      ...
      <BasicButton @click="redirectTo(resource + '.create')" :label="'Nuevo Registro'" :icon="'plus'" :color="'blue'"
        class="text-sm" />
    </div>
        ...
        <div class="w-full sm:max-w-md  rounded-md shadow-md mx-6 sm:m-0 bg-white/95 backdrop-blur-sm ">
          ...
          <div class="p-4 ">
            <div v-for="relationship in relationships">
              <FormSelectHorizontal :id="relationship.id" :label="relationship.label" v-model="filters[relationship.id]"
                :name="relationship.id" :resource="relationship.resource" />
            </div>
            <div v-for="field in fields" class="">
              <FormInputHorizontal :id="field.name" :label="field.label" v-model="filters[field.name]"
                :type="field.filter === 'date' ? 'date' : 'search'" />
            </div>
          </div>
      ...
      <table class=" w-full border-separate border-spacing-y-1.5">
        <thead>
          <tr class="bg-white h-12 hover:bg-gray-50">
            <th v-for="relationship in relationships" class="text-sm pl-1 font-medium text-left min-w-max">
              <span class="">{{ relationship.label }}</span>
            </th>
        ...
        <tbody>
          <tr v-for="data in dataReceived" class="bg-white h-12 hover:bg-gray-50">
            <td v-for="relationship in relationships" class="text-sm pl-1 text-gray-700">
              {{ data['relationships'][relationship.resource]['data'][relationship.name] }}</td>
            <td v-for="field in fields" class="text-sm pl-1 text-gray-700">
